import { request } from '@glow/rest-api-client';
import type { JSONSchema7 } from 'json-schema';
import type { NodeParameterValueType } from 'glow-workflow';
...
export const getSchemaPreview = async (
	...
	const path = ['schemas', nodeType.replace('@glow/', ''), versionString, resource, operation]
		.filter(Boolean)
		.join('/');
