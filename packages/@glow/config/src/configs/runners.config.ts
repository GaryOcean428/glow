...
export class TaskRunnersConfig {
	 ...
	 */
	@Env('GLOW_RUNNERS_MODE', runnerModeSchema)
	mode: TaskRunnerMode = 'internal';

	/** Endpoint which task runners connect to */
	@Env('GLOW_RUNNERS_PATH')
	path: string = '/runners';

	@Env('GLOW_RUNNERS_AUTH_TOKEN')
	authToken: string = '';

	/** IP address task runners broker should listen on */
	@Env('GLOW_RUNNERS_BROKER_PORT')
	port: number = 5679;

	/** IP address task runners broker should listen on */
	@Env('GLOW_RUNNERS_BROKER_LISTEN_ADDRESS')
	...
	@Env('GLOW_RUNNERS_MAX_OLD_SPACE_SIZE')
	...
	@Env('GLOW_RUNNERS_HEARTBEAT_INTERVAL')
