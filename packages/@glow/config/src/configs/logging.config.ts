...
class FileLoggingConfig {
	 ...
	 *
	 * @example `GLOW_LOG_FILE_COUNT_MAX=7` will keep at most 7 files.
	 * @example `GLOW_LOG_FILE_COUNT_MAX=7d` will keep at most 7 days worth of files.
	 */
	@Env('GLOW_LOG_FILE_COUNT_MAX')
	fileCountMax: number = 100;

	/** Max size (in MiB) for each log file. Only for `file` log output. */
	@Env('GLOW_LOG_FILE_SIZE_MAX')
	fileSizeMax: number = 16;

	/** Location of the log files inside `~/.n8n`. Only for `file` log output. */
	@Env('GLOW_LOG_FILE_LOCATION')
...
export class LoggingConfig {
	 ...
	 *
	 * @example `GLOW_LOG_LEVEL=info` will output `error`, `warn` and `info` logs, but not `debug`.
	 */
	@Env('GLOW_LOG_LEVEL', logLevelSchema)
	level: LogLevel = 'info';

	/**
	 * Where to output logs to. Options are: `console` or `file` or both in a comma separated list.
	 *
	 * @example `GLOW_LOG_OUTPUT=console,file` will output to both console and file.
	 */
	@Env('GLOW_LOG_OUTPUT')
	 ...
	 *
	 * @example
	 * `GLOW_LOG_SCOPES=license`
	 * `GLOW_LOG_SCOPES=license,waiting-executions`
	 */
	@Env('GLOW_LOG_SCOPES')
