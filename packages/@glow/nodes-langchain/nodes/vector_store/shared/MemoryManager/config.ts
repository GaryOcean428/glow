...
export function getConfig(): MemoryVectorStoreConfig {
	...
	if (process.env.GLOW_VECTOR_STORE_MAX_MEMORY) {
		const parsed = parseInt(process.env.GLOW_VECTOR_STORE_MAX_MEMORY, 10);
		if (!isNaN(parsed)) {
			maxMemoryMB = parsed;
		}
	}
	...
	if (process.env.GLOW_VECTOR_STORE_TTL_HOURS) {
		const parsed = parseInt(process.env.GLOW_VECTOR_STORE_TTL_HOURS, 10);
		if (!isNaN(parsed)) {
			ttlHours = parsed;
		}
	}
